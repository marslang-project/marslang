...
- `date`, `datetime`, and `duration` kinds with time zones and daylight saving
- standard packages for math, strings, containers, algorithms, statistics, files, paths, CSV, `.env`, INI, and TOML (`std.file`), JSON, the command line, the environment (`std.os`, `std.sys`), and object identity (`std.memory`); see [docs/api](docs/api/README.md)
