//! Windows time zone names to IANA names, from the Unicode CLDR
//! `windowsZones.xml` (Unicode License v3), as bundled by the jiff crate
//! (MIT or Unlicense). `src/date.rs` uses it to name the system time zone on
//! Windows, where the system reports a Windows name such as "China Standard Time".
//! Sorted by Windows name for binary search.
